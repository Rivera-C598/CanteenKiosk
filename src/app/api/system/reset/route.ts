...
export async function POST(request: NextRequest) {
  ...
  if (action === 'clear' && confirm !== 'CLEAR-DB') {
  ...
  if (action === 'clear' || action === 'seed') {
    try {
      ...
      if (action === 'seed') {
        ...
        return NextResponse.json({ ok: true, message: 'Demo data loaded successfully' })
      }
    } catch (error) {
      console.error('Reset Error:', error)
      return NextResponse.json({ error: 'Internal Server Error during data operation' }, { status: 500 })
    }
  }
